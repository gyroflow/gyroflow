...
impl REDSdk {
    ...
    pub fn get_download_url() -> Option<&'static str> {
        if cfg!(target_os = "windows") {
            Some("https://api.gyroflow.xyz/sdk/RED_SDK_Windows_9.0.1.tar.gz")
        } else if cfg!(target_os = "macos") {
            Some("https://api.gyroflow.xyz/sdk/RED_SDK_MacOS_9.0.1.tar.gz")
        } else if cfg!(target_os = "linux") {
            Some("https://api.gyroflow.xyz/sdk/RED_SDK_Linux_9.0.1.tar.gz")
        } else {
            None
        }
    }
