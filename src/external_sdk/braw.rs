...
impl BrawSdk {
    ...
    pub fn get_download_url() -> Option<&'static str> {
        if cfg!(target_os = "windows") {
            Some("https://api.gyroflow.xyz/sdk/Blackmagic_RAW_SDK_Windows_3.4.tar.gz")
        } else if cfg!(target_os = "macos") {
            Some("https://api.gyroflow.xyz/sdk/Blackmagic_RAW_SDK_MacOS_3.4.tar.gz")
        } else if cfg!(target_os = "linux") {
            Some("https://api.gyroflow.xyz/sdk/Blackmagic_RAW_SDK_Linux_3.4.tar.gz")
        } else {
            None
        }
    }
