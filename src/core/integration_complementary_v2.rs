...
const ANGULAR_VELOCITY_THRESHOLD: f64 = 0.03; // 1,7 deg/s
const ACCELERATION_THRESHOLD: f64 = 0.1;
const DELTA_ANGULAR_VELOCITY_THRESHOLD: f64 = 0.01;
const GRAV_AUTOSCALE_THRESHOLD: f64 = 1.0; // apply autoscaling when acceleration within 1 m/s^2 of GRAVITY
...
impl ComplementaryFilter {
    ...
    pub fn update(&mut self, ax: f64, ay: f64, az: f64, wx: f64, wy: f64, wz: f64, dt: f64) {
        ...
        if self.do_gravity_autoscale {
            self.autoscale_gravity(ax, ay, az, wx, wy, wz)
        }
    ...
    fn autoscale_gravity(&mut self, ax: f64, ay: f64, az: f64, wx: f64, wy: f64, wz: f64) {
        self.steady_state = self.check_state(ax, ay, az, wx, wy, wz);
