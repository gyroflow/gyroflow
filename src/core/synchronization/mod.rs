...
impl PoseEstimator {
    ...
    pub fn find_offsets_rssync<F: Fn(f64) + Sync>(&self, ranges: &[(i64, i64)], initial_offset: f64, search_size: f64, params: &ComputeParams, progress_cb: F, cancel_flag: Arc<AtomicBool>) -> Vec<(f64, f64, f64)> { // Vec<(timestamp, offset, cost)>
        ...
        // dbg!(FindOffsetsRssync::new(ranges, &points, initial_offset, search_size, params, progress_cb, cancel_flag).guess_orient());
        FindOffsetsRssync::new(ranges, &points, initial_offset, search_size, params, progress_cb, cancel_flag).full_sync();
        Vec::new()
    }
