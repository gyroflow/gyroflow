...
: "${QT_DIR:=$PROJECT_DIR/ext/6.3.1/macos}"
...
if [ "$1" == "build-universal" ] || [ "$1" == "deploy-universal" ]; then
    ...
    #export LD_LIBRARY_PATH="$PROJECT_DIR/ext/6.3.1/macos/lib"
...
if [ "$1" == "deploy" ] || [ "$1" == "deploy-universal" ]; then
    ...
    hdiutil create "$PROJECT_DIR/_deployment/_binaries/Gyroflow-mac-universal.dmg" -volname "Gyroflow v1.2.0" -fs HFS+ -srcfolder "$PROJECT_DIR/_deployment/_binaries/mac/" -ov -format UDZO -imagekey zlib-level=9
