...
pub fn draw_pixel(in_pix: Vec4, x: f32, y: f32, is_input: bool, params: &KernelParams, _coeffs: &[f32], drawing: &DrawingType, _sampler: SamplerType) -> Vec4 {
...
pub fn draw_safe_area(in_pix: Vec4, x: f32, y: f32, params: &KernelParams) -> Vec4 {
    ...
    if !is_safe_area {
        ...
        if is_border {
           pix = vec4(40.0, 40.0, 40.0, 255.0) / BG_SCALER;
        }
    }
    pix
}
...
fn remap_colorrange(px: Vec4, is_y: bool) -> Vec4 {
    if is_y { (16.0 / BG_SCALER) + (px * 0.85882352) } // (235 - 16) / 255
    else    { (16.0 / BG_SCALER) + (px * 0.87843137) } // (240 - 16) / 255
}
    ...
    if drawing_enabled {
        if src_pos.y >= params.source_rect.y as f32 && src_pos.y < (params.source_rect.y + params.source_rect.w) as f32 {
            if src_pos.x >= params.source_rect.x as f32 && src_pos.x < (params.source_rect.x + params.source_rect.z) as f32 {
                pixel = draw_pixel(pixel, src_pos.x, src_pos.y, true, params, coeffs, drawing, sampler);
            }
        }
        pixel = draw_pixel(pixel, out_pos.x, out_pos.y, false, params, coeffs, drawing, sampler);
