...
pub fn generate_stmaps(stab: &StabilizationManager, per_frame: bool) -> impl Iterator<Item = (String, usize, Vec<u8>, Vec<u8>)> { // (frame, undistort, redistort)
    ...
    (0..compute_params.frame_count).map(move |frame| {
        ...
        let (camera_matrix, distortion_coeffs, output_camera_matrix, rotations, is, mesh) = FrameTransform::at_timestamp_for_points(&compute_params, &bbox, timestamp, Some(frame), false);
        let undistorted_bbox = undistort_points(&bbox, camera_matrix, &distortion_coeffs, rotations[0], None, Some(output_camera_matrix), Some(rotations), &compute_params, 1.0, timestamp, is, mesh);
        ...
        let dist = parallel_exr(width, height, |x, y| {
            ...
            let (camera_matrix, distortion_coeffs, output_camera_matrix, rotations, is, mesh) = FrameTransform::at_timestamp_for_points(&compute_params, &distorted, timestamp, Some(frame), true);
            undistort_points(&distorted, camera_matrix, &distortion_coeffs, rotations[0], None, Some(output_camera_matrix), Some(rotations), &compute_params, 1.0, timestamp, is, mesh).first().copied()
        });
