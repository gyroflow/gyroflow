...
pub fn generate_stmaps(stab: &StabilizationManager, per_frame: bool) -> impl Iterator<Item = (String, usize, Vec<u8>, Vec<u8>)> { // (frame, undistort, redistort)
    ...
    (0..compute_params.frame_count).map(move |frame| {
        ...
        let undistorted_bbox = undistort_points(&bbox, camera_matrix, &distortion_coeffs, rotations[0], None, None, Some(rotations), &compute_params, 1.0, timestamp, is, mesh);
        ...
        let dist = parallel_exr(width, height, |x, y| {
            ...
            undistort_points(&distorted, camera_matrix, &distortion_coeffs, rotations[0], None, None, Some(rotations), &compute_params, 1.0, timestamp, is, mesh).first().copied()
        });
