...
if [ "$1" != "CI" ]; then
    ...
    `python3 -m site --user-base`/bin/aqt install-qt mac desktop 6.2.2
...
curl -L https://sourceforge.net/projects/avbuild/files/macOS/ffmpeg-4.4-macOS-default.tar.xz/download -o ffmpeg.tar.xz
7z x ffmpeg.tar.xz
7z x ffmpeg.tar
mkdir -p ffmpeg-x86_64/lib
mkdir -p ffmpeg-arm64/lib
cd ffmpeg-4.4-macOS-default
